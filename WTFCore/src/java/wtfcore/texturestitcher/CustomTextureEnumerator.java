...
public class CustomTextureEnumerator implements IIconRegister{
	...
	//	WTFCore.log.info(String.format("CustomTextureEnumerator.registerIcon for" + resourceLocation));
		...
		String newresourceLocation = ClientProxy.overlayDomain + ":" +resource.getResourcePath();
	...
	public void handleTextureReload(TextureStitchEvent.Pre event) {
		...
		WTFCore.log.info("CustomTextureEnumerator.handleTextureReload:Iterating to find instancesof OverlaidBlock");

		Iterator<Block> iterator = Block.blockRegistry.iterator();
		while (iterator.hasNext()) {
			Block block = iterator.next();
			if (block instanceof IAlphaMaskedBlock || ClientProxy.retextureBlock.contains(block)) {
